...
### 第6章: コンテナ化の第一歩 Dockerfileの書き方
- **スラッグ**: `week3-01-docker-container`
- **概要**:
    - アプリケーションを「どこでも動く」状態にするコンテナ技術の基礎。
    - `python:3.11-slim` ベースイメージの選定理由と軽量化。
    - **実践**: FastAPIアプリをDockerコンテナにパッケージングし、ローカルで起動確認する。
- **実装のポイント**:
    - `.dockerignore` で不要ファイルを除外（`node_modules`除外と同じ感覚）。
    - 環境変数（API Key）の注入方法。

### 第7章: AIの「健康診断」：Langfuseで観測性を手に入れる
- **スラッグ**: `week3-02-observability-langfuse`
- **概要**:
...
### 第8章: 「AIの通信簿」：Langfuseを使った評価とプロンプト管理
- **スラッグ**: `week3-03-evaluation-prompt-management`
- **概要**:
...
### 第9章: クラウドへのデプロイとUIの接続
- **スラッグ**: `week4-01-deploy-and-ui`
- **概要**:
    - **デプロイ**: Week 3で作成したコンテナを、Google Cloud RunやAWS App Runnerのようなサーバーレス環境にデプロイする。
