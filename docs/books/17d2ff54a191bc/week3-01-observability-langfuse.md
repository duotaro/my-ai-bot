...
**Datadog**、**New Relic**、**Sentry** といったAPM（Application Performance Monitoring）ツールをご存知の方もいるでしょう。これらはアプリケーションの挙動を監視するためのツールで、「このAPIエンドポイントの平均レスポンスタイムは200ms」「エラー率は0.1%」といった情報をダッシュボードで可視化してくれます。
...
(⚠️ `source .venv/bin/activate` で仮想環境を有効化するのを忘れずに！)
...
これで、あなたのAIサーバーは「何を考え、どれだけコストがかかっているのか」が丸見えになりました。
