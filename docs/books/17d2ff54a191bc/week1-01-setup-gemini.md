...
それでは、プロジェクトルートに `main.py` を作成し、以下のコードを書いてみましょう。HTTPクライアントでAPIを叩く感覚に近いです。
...
次章では、AIアプリ最大の難所である **「会話の記憶（コンテキスト管理）」** について、「ステートレス」という概念をキーワードに紐解いていきます。
