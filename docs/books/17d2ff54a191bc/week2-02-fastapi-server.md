...
これをWebアプリケーションやスマートフォンアプリから利用できるようにするには、**Web API**として機能を公開する必要があります。何らかのフレームワーク（Express、Spring Boot、Ginなど）を使ってAPIサーバーを立てた経験がある方もいるでしょう。
...
来週からは、いよいよ**GenAIOps**の本丸、「Ops（運用）」の世界に足を踏み入れます。AIが「何を考え、どれだけコストがかかっているのか」を可視化する **観測性（Observability）** の技術を学んでいきましょう。お楽しみに。
